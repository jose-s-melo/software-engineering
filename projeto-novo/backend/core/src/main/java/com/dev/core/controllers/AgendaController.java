...
@Tag(
        name = "Agenda",
        description = "Endpoints para gerenciamento de horários de trabalho dos barbeiros")
public class AgendaController {
    ...
    @Operation(
            ...
            description =
                    "Recebe os horários de trabalho semanais. Se o barbeiro já tiver uma agenda, ela será atualizada (Upsert).")
    public ResponseEntity<AgendaResponseDTO> createOrUpdateAgenda(
            @RequestBody @Valid AgendaRequestDTO body) {

        Agenda agendaSalva = agendaService.createOrUpdateAgenda(body);

        List<HorarioAtendimentoDTO> horariosDTO =
                agendaSalva.getHorariosDaSemana().stream()
                        .map(
                                h ->
                                        new HorarioAtendimentoDTO(
                                                h.getDiaDaSemana(),
                                                h.getHorarioAbertura(),
                                                h.getHorarioFechamento()))
                        .collect(Collectors.toList());

        AgendaResponseDTO response =
                new AgendaResponseDTO(
                        agendaSalva.getId(),
