...
public class AgendaService {
    ...
    public Agenda createOrUpdateAgenda(AgendaRequestDTO dto) {
        ...
        User barbeiro =
                userRepository.findById(dto.barbeiroId()).orElseThrow(UserNotFoundException::new);

        if (barbeiro.getRole() != UserRole.BARBEIRO) {
            throw new InvalidAgendaException(
                    "Apenas usuários com perfil BARBEIRO podem ter uma agenda.");
        }
        ...
        Agenda agenda = agendaRepository.findByBarbeiro(barbeiro).orElse(new Agenda());

        agenda.setBarbeiro(barbeiro);

        List<HorarioAtendimento> novosHorarios =
                dto.horariosDaSemana() != null
                        ? dto.horariosDaSemana().stream()
    ...
    private void validarHorarios(List<HorarioAtendimentoDTO> horarios) {
        Map<DayOfWeek, List<HorarioAtendimentoDTO>> horariosPorDia =
                horarios.stream()
                ...
                if (!atual.horarioAbertura().isBefore(atual.horarioFechamento())) {
                    throw new InvalidAgendaException(
                            String.format(
                                    ...
                                    atual.diaDaSemana(),
                                    atual.horarioAbertura(),
                                    atual.horarioFechamento()));
                }
