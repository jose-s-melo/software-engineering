...
@Tag(
        name = "Agendamentos",
        description = "Endpoints para gerenciamento da agenda de horários do barbeiro")
@RestController
...
public class AgendamentoController {
    ...
    @Operation(
            summary = "Criar ou atualizar horários disponíveis para um dia",
            description =
                    "Disponibiliza uma lista de horários para uma data específica. O dia da semana é calculado automaticamente.")
    @ApiResponses(
            value = {
    ...
    @PostMapping
    public ResponseEntity<AgendamentoResponseDTO> criarAgendaDisponibilidade(
            @RequestBody AgendamentoRequestDTO dto) {
        // O Service processa os dados usando os campos do DTO de entrada
        Agendamento agendamento =
                agendamentoService.criarAgenda(dto.data(), dto.horariosDisponiveis());
    ...
    @Operation(
            summary = "Buscar os horários disponíveis por data",
            description =
                    "Retorna a agenda e os horários que ainda não foram preenchidos em um dia específico.")
    @ApiResponses(
            value = {
                @ApiResponse(responseCode = "200", description = "Agenda localizada com sucesso"),
                @ApiResponse(
                        responseCode = "404",
                        description = "Nenhum horário cadastrado para a data informada")
            })
    ...
    /**
     * Método auxiliar privado para converter a Entidade JPA para o DTO de saída. Dica: Em projetos
     * grandes, você pode substituir isso por uma biblioteca como MapStruct ou ModelMapper.
     */
