...
public class AuthService {
    ...
    @Autowired private ForgotPasswordEntityRepository forgotPasswordEntityRepository;
    ...
    public TokenResponseDTO login(LoginRequestDTO dto) {
        ...
        return new TokenResponseDTO(
                tokenService.generateToken((UserDetails) authentication.getPrincipal()),
                "Success",
    ...
    public void forgotPassword(ForgotPasswordRequestDTO body) {
        ...
        String randomCode =
                random.ints(length, 0, permittedChars.length())
                        .mapToObj(permittedChars::charAt)
                        .collect(StringBuilder::new, StringBuilder::append, StringBuilder::append)
                        .toString();

        emailService.sendEmail(
                new SendEmailDTO(body.email(), randomCode, "Código para mudar senha"));
    ...
    public void confirmForgotPassword(ConfirmForgotPasswordRequestDTO body) {
        ...
        ForgotPasswordEntity forgotPasswordEntity =
                forgotPasswordEntityRepository
    ...
    private void invalidateCodes() {
        ...
        for (ForgotPasswordEntity forgotPasswordEntity : all) {
            Duration duration =
                    Duration.between(forgotPasswordEntity.getCreatedAt(), Instant.now());
