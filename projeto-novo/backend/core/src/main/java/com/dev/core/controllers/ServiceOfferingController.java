...
@Tag(
        ...
        description =
                "Endpoints responsáveis pelo gerenciamento de serviços oferecidos pelas barbearias")
public class ServiceOfferingController {
    ...
    public ResponseEntity<ServiceOfferingResponseDTO> create(
            @PathVariable UUID barbershopId, @RequestBody ServiceOfferingCreateDTO body) {

        return ResponseEntity.status(HttpStatus.CREATED).body(service.create(barbershopId, body));
    }
                        ...
                        content =
                                @Content(
                                        ...
                                        schema =
                                                @Schema(
                                                        implementation =
                                                                ServiceOfferingResponseDTO.class)))
            })
    ...
    public ResponseEntity<ServiceOfferingResponseDTO> findById(
            @PathVariable UUID barbershopId, @PathVariable UUID serviceId) {
    ...
    @ApiResponses(
            value = {
                ...
                @ApiResponse(responseCode = "400", description = "Dados inválidos"),
                @ApiResponse(responseCode = "404", description = "Serviço não encontrado"),
                @ApiResponse(responseCode = "409", description = "Conflito de nome de serviço")
            })
    public ResponseEntity<ServiceOfferingResponseDTO> update(
        ...
        return ResponseEntity.ok(service.update(barbershopId, serviceId, body));
    }
